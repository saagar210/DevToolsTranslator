...
function parseArgs(argv) {
  const args = { dryRun: false, version: null, fromStage: null, toStage: null };
...
function main() {
  ...
  if (!fs.existsSync(evalPath)) {
    process.stdout.write(
      `${JSON.stringify(
        {
          status: 'error',
          error_code: 'missing_stage_evaluation',
      ...
      `${JSON.stringify(
        {
          ...
          message: 'Extension stage advance blocked by rollout controller.',
        },
        null,
        2,
      )}\n`,
  ...
  const approvalPath = path.join(outputDir, `advance-${args.fromStage}-to-${args.toStage}.v1.json`);
